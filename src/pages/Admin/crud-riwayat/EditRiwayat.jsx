...
const EditRiwayat = () => {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				...
				const riwayatResponse = await axios.get(`http://localhost:9000/riwayat/${id}`, {
					headers: { Authorization: `Bearer ${token}` },
				});
	...
	const handleSubmit = async (e) => {
		...
		try {
			const token = localStorage.getItem("token");
			const response = await axios.put(`http://localhost:9000/riwayat/${id}`, formData, {
				headers: { Authorization: `Bearer ${token}` },
			});

			if (response.status === 200) {
				alert("Riwayat berhasil diperbarui");
				// Redirect to the riwayat list page after submitting
	...
	return (
		<div className="p-6 min-h-screen flex justify-center items-center">
			...
			>
				<h1 className="text-3xl font-bold mb-6 text-center">Edit Riwayat Aset</h1>
